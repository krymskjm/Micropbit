...
#define GPIO0 ((NRF_GPIO_REGS0*)0x50000000) //gpio 0 base adress		// Datasheet page 22
#define GPIO1 ((NRF_GPIO_REGS1*)0x50000300) //gpio 1 base adress		// Datasheet page 22
...
typedef struct {
	...
	volatile uint32_t RESERVED1_0[118];		// maybe 93?	// 118
...
typedef struct {
	...
	volatile uint32_t LATCH;
	volatile uint32_t DETECTMODE;
	volatile uint32_t RESERVED1_1[118];		// maybe 93 not 118?
...
int main(){
	...
	GPIO0->PIN_CNF[23] = 0; // button B		// 23 from schematics
	...
	while(1){
		...
		/* Check if button B (23) is pressed;
		 * turn on LED matrix if it is. */
		if (!(GPIO0->IN & (1 << 23))) {		// btnB pressed (bit 23 is active low)
			GPIO0->OUTSET = 0x1 << 21;
			GPIO0->OUTSET = 0x1 << 22;
			GPIO0->OUTSET = 0x1 << 15;
			GPIO0->OUTSET = 0x1 << 24;
		...
		/* Check if button A(14) is pressed;
